...
-- Two separate statements on purpose: some MySQL/MariaDB builds (common on
-- free hosts) do not parse a single ALTER TABLE that combines ADD COLUMN
-- and ADD CONSTRAINT ... CHECK with a comma. If the column already exists
-- from a previous partial run, this first statement is the only one that
-- will error (1060 Duplicate column) -- skip it and run the rest.
ALTER TABLE perfumes ADD COLUMN gender VARCHAR(10) NOT NULL DEFAULT 'unisex';
ALTER TABLE perfumes ADD CONSTRAINT ck_perfumes_gender CHECK (gender IN ('men', 'women', 'unisex'));
